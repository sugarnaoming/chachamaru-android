...
class DatabaseInit(private val db: SQLiteDatabase) {
  fun run() {
    db.run {
      execSQL(DatabaseInit.invokeForeignKeySql)
      execSQL(DatabaseInit.createGroupListTableSql)
      execSQL(DatabaseInit.createUrlListTableSql)
      execSQL(DatabaseInit.insertGroupListSql)
      execSQL(DatabaseInit.insertUrlListSql())
    }
  }

  companion object {
    private val invokeForeignKeySql = "PRAGMA foreign_keys = ON"
    private val createGroupListTableSql = "create table grouplist(" +
        "_id integer primary key autoincrement," +
        "group_name text not null unique)"
    private val createUrlListTableSql = "create table urllist(" +
        "_id integer primary key autoincrement," +
        "title text not null," +
    ...
    private val insertGroupListSql = "insert into grouplist(group_name)" +
        "values('はてなブックマーク/新着')," +
        "('はてなブックマーク/人気')," +
        "('Qiita')," +
        "('Web')," +
